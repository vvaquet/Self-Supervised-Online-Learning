...
def get_weighted_distance(dist):
    return min(1/dist, 99999999)

class KnnUnsupervisedC2:
    ...
    def metric(self, prediction, closest_labels, closest_dists):
        ...
        if np.isnan(certainty_t/(certainty_t+certainty_f)):
            print(certainty_t,certainty_f)

        return certainty_t/(certainty_t+certainty_f)
