...
class KnnUnsupervisedC3:
    ...
    def metric(self, prediction, closest_labels, closest_dists):
        ...
        return (certainty_t -  certainty_f) /(2*self.k)
    ...
    def predict_with_certainty(self, datapoint):
        ...
        sorted_idcs = np.argsort(distances)
        sorted_labels = np.array(self.labels)[sorted_idcs]
        sorted_dists = distances[sorted_idcs]
        idcs_same_cls = sorted_labels == prediction
        idcs_other_cls = sorted_labels != prediction

        labels_same = sorted_labels[idcs_same_cls]
        labels_other = sorted_labels[idcs_other_cls]
        dists_same = sorted_dists[idcs_same_cls]
        dists_other = sorted_dists[idcs_other_cls]

        close_labels = np.hstack((labels_same[:min(self.k, len(labels_same))], labels_other[:min(self.k, len(labels_other))]))
        close_dists = np.hstack((dists_same[:min(self.k, len(dists_same))], dists_other[:min(self.k, len(dists_other))]))

        certainty = self.metric(prediction,close_labels, close_dists)
